<?php
	...
	class Addresses  {
		...
		{
			$sql = "select a.id,a.addresses Addresses,a.City City, a.State State,a.Zip Zip,a.Country Country,
			...
			if (!empty($row['id'])) {
				$sql = "UPDATE 2014Spring_Addresses
							SET Addresses='$row2[Addresses]', City='$row2[City]',
								State='$row2[State]', Zip='$row2[Zip]'
						WHERE id = $row2[id]
						";
			}else{
				$sql = "INSERT INTO 2014Spring_Addresses
						(Addresses,City,State,Zip,Users_id,Country)
						VALUES ('$row2[Addresses]', '$row2[City]', '$row2[State]', '$row2[Zip]', 
		...
		{
			return array( 'id' => null,'Addresses'=>null,
			'City'=>null,'State'=>null,'Zip'=>null,'AddressType'=>null,'Users_id'=>null,'Country'=>null ,'User'=>null);
		}
